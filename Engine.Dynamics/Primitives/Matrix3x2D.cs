    ...
    : IMatrix<Matrix3x2D, Vector2D>
{
    ...
    /// </summary>
    public static readonly Matrix3x2D Empty = new(0d, 0d, 0d, 0d, 0d, 0d);

    /// <summary>
    /// The na n
    /// </summary>
    public static readonly Matrix3x2D NaN = new(double.NaN, double.NaN, double.NaN, double.NaN, double.NaN, double.NaN);

    /// <summary>
    /// An Identity <see cref="Matrix3x2D"/>.
    /// </summary>
    public static readonly Matrix3x2D Identity = new(1d, 0d, 0d, 1d, 0d, 0d) { type = MatrixTypes.Identity };
    ...
    public double M22
    {
        readonly get { return type == MatrixTypes.Identity ? 1.0f : m1x1; }
        set
        {
            if (type == MatrixTypes.Identity)
            {
    ...
    private readonly bool IsDistinguishedIdentity => type == MatrixTypes.Identity;

    /// <summary>
    /// Tests whether or not a given transform is an identity transform
    /// </summary>
    [IgnoreDataMember, XmlIgnore, SoapIgnore]
    public readonly bool IsIdentity => type == MatrixTypes.Identity || IsMatrixIdentityEpsilon(m0x0, m0x1, m1x0, m1x1, offsetX, offsetY);
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public override readonly bool Equals([AllowNull] object obj) => obj is Matrix3x2D d && Equals(this, d);
    ...
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public readonly bool Equals(Matrix3x2D value) => Equals(this, value);
    #endregion
    ...
    /// MultiplyVector
    /// </summary>
    public readonly void MultiplyVector(ref double x, ref double y)
    {
        switch (type)
        {
            case MatrixTypes.Identity:
    ...
    /// MultiplyVector
    /// </summary>
    public readonly void MultiplyVector(ref Vector2D vector)
    {
        switch (type)
        {
            case MatrixTypes.Identity:
    ...
    /// MultiplyPoint
    /// </summary>
    public readonly void MultiplyPoint(ref double x, ref double y)
    {
        switch (type)
        {
            case MatrixTypes.Identity:
    ...
    /// MultiplyPoint
    /// </summary>
    public readonly void MultiplyPoint(ref Point2D point)
    {
        switch (type)
        {
            case MatrixTypes.Identity:
    ...
    /// </summary>
    public readonly double Determinant => type switch
    {
    ...
    public readonly IEnumerator<IEnumerable<double>> GetEnumerator() => new List<List<double>>
        {
            new() { m0x0, m0x1, offsetX },
            new() { m1x0, m1x1, offsetY },
        }.GetEnumerator();
