    ...
    : IPrimitive, IMatrix<Transform2D, Vector2D>, IEquatable<Transform2D>
{
    ...
    /// The empty
    /// </summary>
    public static readonly Transform2D Empty = new(0d, 0d, 0d, 0d, 0d, 0d);

    /// <summary>
    /// The na n
    /// </summary>
    public static readonly Transform2D NaN = new(double.NaN, double.NaN, double.NaN, double.NaN, double.NaN, double.NaN);

    /// <summary>
    /// The identity.
    /// </summary>
    public static readonly Transform2D Identity = new(0d, 0d, 0d, 0d, 1d, 1d);
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public readonly void Deconstruct(out double x, out double y, out double skewX, out double skewY, out double scaleX, out double scaleY)
    {
        x = X;
        y = Y;
        skewX = SkewX;
    ...
    [RefreshProperties(RefreshProperties.All)]
    public Point2D Location { readonly get { return new Point2D(X, Y); } set { (X, Y) = value; } }
    ...
    [TypeConverter(typeof(Point2DConverter))]
    [RefreshProperties(RefreshProperties.All)]
    public Vector2D Skew { readonly get { return new Vector2D(X, Y); } set { (SkewX, SkewY) = value; } }
    ...
    [TypeConverter(typeof(Point2DConverter))]
    [RefreshProperties(RefreshProperties.All)]
    public Size2D Scale { readonly get { return new Size2D(X, Y); } set { (ScaleX, ScaleY) = value; } }
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public override readonly bool Equals([AllowNull] object obj) => obj is Ellipse2D o && Equals(o);
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public readonly bool Equals([AllowNull] Transform2D other) => X == other.X && Y == other.Y && SkewX == other.SkewX && SkewY == other.SkewY && ScaleX == other.ScaleX && ScaleY == other.ScaleY;
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Transform2D FromValueTuple((double X, double Y, double SkewX, double SkewY, double ScaleX, double ScaleY) tuple) => new(tuple);
    #endregion
    ...
    public static Transform2D FromMatrix(Matrix3x2D matrix)
    {
        ...
        var scaleY = (skewX > -Floats<double>.Quart && skewX < Floats<double>.Quart) ? matrix.M22 / Cos(skewX) : -matrix.M21 / Sin(skewX);
        var scaleX = (skewY > -Floats<double>.Quart && skewY < Floats<double>.Quart) ? matrix.M11 / Cos(skewY) : matrix.M12 / Sin(skewY);
    ...
    /// </returns>
    public readonly Matrix3x2D ToMatrix() => new(ScaleX * Cos(SkewY), ScaleX * Sin(SkewY), -ScaleY * Sin(SkewX), ScaleY * Cos(SkewX), X, Y);
    #endregion Factories
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public readonly IEnumerator<IEnumerable<double>> GetEnumerator() => throw new NotImplementedException();
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public override readonly int GetHashCode() => HashCode.Combine(X, Y, SkewX, SkewY, ScaleX, ScaleY);
