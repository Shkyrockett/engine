...
public static class BezierUtil
{
    ...
    /// http://pomax.github.io/bezierinfo/
    /// </acknowledgment>
    public static readonly double[] Tvalues =
    [
        // 
    ...
    /// http://pomax.github.io/bezierinfo/
    /// </acknowledgment>
    public static readonly double[] Cvalues =
    [
        //
    ...
    {
        ArgumentNullException.ThrowIfNull(derivativeFn);
    ...
    public static Arc2D Getccenter(Point2D p1, Point2D p2, Point2D p3)
    {
        ...
        var dx2 = p3.X - p2.X;
        var dy2 = p3.Y - p2.Y;
        var dx1p = (dx1 * Cos(Floats<double>.Quart)) - (dy1 * Sin(Floats<double>.Quart));
        var dy1p = (dx1 * Sin(Floats<double>.Quart)) + (dy1 * Cos(Floats<double>.Quart));
        var dx2p = (dx2 * Cos(Floats<double>.Quart)) - (dy2 * Sin(Floats<double>.Quart));
        var dy2p = (dx2 * Sin(Floats<double>.Quart)) + (dy2 * Cos(Floats<double>.Quart));
        // chord midpoints
    ...
    //[DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Point2D Copy(Point2D point) => new(point);
    ...
    //[DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Point3D Copy(Point3D point) => new(point);
    ...
    public static Shape1 MakeShape(Bezier forward, Bezier back)
    {
        ...
        var shape = new Shape1(
            ...
            back: back,
            endcap: end,
            bbox: FindBoundingBox([start, forward, back, end])
          );
