    ...
    : IFormattable
{
    ...
    public Polynomialx Normalize()
    {
        ...
        {
            if (Abs(Coefficients[i]) > double.Epsilon)
            {
                order = i;
                high = Coefficients[i];
            }
    ...
    public double? Bisection(double min, double max)
    {
        ...
        if (Abs(minValue) <= Floats<double>.Tolerance)
        {
            result = min;
        }
        else if (Abs(maxValue) <= Floats<double>.Tolerance)
        {
            result = max;
        }
        else if (minValue * maxValue <= 0)
        {
            ...
            double tmp2 = Floats<double>.LN10 * Floats<double>.Accuracy;
            ...
            for (var i = 0; i < iters; i++)
            {
                ...
                if (Abs(value) <= Floats<double>.Tolerance)
                {
                    break;
                }
    ...
    {
        var points = Derivate().SolveOrFindRealRoots().Where(t => t > x0 && t < x1).Concat([x0, x1]);
        var first = true;
        minY = 0;
        maxY = 0;
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public IEnumerable<double> SolveOrFindRealRoots() => CanSolveRealRoots ? SolveRealRoots() : FindRoots().Where(c => Abs(c.Imaginary) < double.Epsilon).Select(c => c.Real);
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public IEnumerable<double> SolveRealRoots() => SolveRealRoots([.. Coefficients]);
    ...
    public Complex[] FindRoots()
    {
        ...
        bool closeEnough()
        {
            ...
            {
                var c = R0[i] - R1[i];
                if (Abs(c.Real) > double.Epsilon || Abs(c.Imaginary) > double.Epsilon)
                {
                    return false;
                }
            }
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public Polynomialx Trim() => Trim(double.Epsilon);
    ...
    public override bool Equals(object obj)
    {
        ...
        for (var i = 0; i < Coefficients.Count; i++)
        {
            if (Abs(Coefficients[i] - p.Coefficients[i]) > double.Epsilon)
            {
                return false;
            }
        }
    ...
    public virtual string ConvertToString(string format, IFormatProvider formatProvider)
    {
        ...
        for (var i = 0; i < Coefficients.Count; i++)
        {
            ...
            if (i > 0 && (Abs(val) - 1) < double.Epsilon)
            {
                if (val < 0)
                {
                    sb.Append('-');
    ...
    /// <param name="right">The right.</param>
    /// <returns></returns>
    public static Polynomialx Multiply(Polynomialx left, Polynomialx right) => left * right;
    ...
    /// <param name="right">The right.</param>
    /// <returns></returns>
    public static Polynomialx Multiply(double left, Polynomialx right) => left * right;
    ...
    /// <param name="right">The right.</param>
    /// <returns></returns>
    public static Polynomialx Multiply(Polynomialx left, double right) => left * right;
    ...
    /// <param name="right">The right.</param>
    /// <returns></returns>
    public static Polynomialx Divide(Polynomialx left, double right) => left / right;
