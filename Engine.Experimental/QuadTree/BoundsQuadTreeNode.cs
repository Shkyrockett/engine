...
namespace Engine.Experimental
{
        ...
        where T : IBoundable
    {
        ...
        /// </summary>
        private List<BoundsQuadTreeNode<T>> nodes = new List<BoundsQuadTreeNode<T>>(4);
        #endregion Fields
        ...
        {
            foreach (BoundsQuadTreeNode<T> node in nodes) node.EchoSubTreeContents(funFind);
        ...
        public void Delete(Rectangle2D queryArea, BoundsQuadTree<T>.Find funFind)
        {
            if (Contents != null)
            {
                foreach (T item in Contents)
                {
                    if (queryArea.IntersectsWith(item.Bounds) && funFind(item)) Contents.Remove(item);
                }
            }
            foreach (BoundsQuadTreeNode<T> node in nodes)
            {
