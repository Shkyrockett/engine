...
public abstract class CurveFitBase
{
    ...
    /// </summary>
    protected readonly List<Point2D> points = new(256);
    ...
    /// </summary>
    protected readonly List<double> arclen = new(256);
    ...
    /// curve.
    /// </summary>
    protected readonly List<double> u = new(256);
    ...
    protected bool FitCurve(int first, int last, Vector2D tanL, Vector2D tanR, out CubicBezier2D curve, out int split)
    {
        ...
        {
            split = 0;
            ArcLengthParameterize(first, last); // Initially start u with a simple chord-length parameterization.
            curve = default;
            for (var i = 0; i < MaxItterations + 1; i++)
            {
                ...
                if (i != 0)
                {
                    Reparametrize(first, last, curve);
                }
    ...
    /// </summary>
    protected void ArcLengthParameterize(int first, int last)
    {
        u.Clear();
    ...
    protected CubicBezier2D GenerateCubicBezier(int first, int last, Vector2D tanL, Vector2D tanR, double epsilon = double.Epsilon)
    {
        ...
        for (var i = 1; i < nPts; i++)
        {
            ...
            var a0 = tanL * t1;
            var a1 = tanR * t2;
            c00 += Operations.DotProduct<double>(a0, a0);
            c01 += Operations.DotProduct<double>(a0, a1);
            c11 += Operations.DotProduct<double>(a1, a1);

            // X matrix.
            x0 += Operations.DotProduct<double>((Point2D)a0, v);
            x1 += Operations.DotProduct<double>((Point2D)a1, v);
        }
    ...
    /// </summary>
    protected void Reparametrize(int first, int last, CubicBezier2D curve, double epsilon = double.Epsilon)
    {
