    ...
    : ICollection<GraphicItem>
{
    ...
    public List<GraphicItem> this[Rectangle2D area] => new(from shape in Items where (shape?.Shape?.Bounds is not null) && shape.VisibleTest(area) select shape);
    ...
    /// <returns>One element of type List{GraphicItem}.</returns>
    public List<GraphicItem> this[Point2D point] => new(from shape in Items where (shape?.Shape?.Bounds is not null) && shape.Shape.Bounds.Contains(point) && shape.Shape.Contains(point) select shape);
    #endregion Indexers
    ...
    public List<GraphicItem> SelectItems(Point2D point) => new(from shape in Items where shape.Shape.Bounds.IntersectsWith(VisibleBounds) && shape.Contains(point) select shape);
