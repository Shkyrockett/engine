...
public static partial class Conversions
{
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static CircularArc2D CircleToCircularArc(double x, double y, double r) => new(x, y, r, 0, Tau);
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Ellipse2D CircleToEllipse(double x, double y, double r) => new(x, y, r, r, 0);
        ...
        double startAngle, double sweepAngle)
    {
        ...
        var segs = Ceiling(Abs(sweepAngle) / Floats<double>.Hau);
        var theta = sweepAngle / segs;  /* arc size of each segment */
        var tanT2 = Tan(theta * Floats<double>.OneHalf);
        var alpha = Sin(theta) * (Sqrt(4 + (3 * tanT2 * tanT2)) - 1) * Floats<double>.OneThird;
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static QuadraticBezier2D LineSegmentToQuadraticBezier(double x0, double y0, double x1, double y1) => new(new Point2D(x0, y0), Lerp(x0, y0, x1, y1, Floats<double>.OneHalf), new Point2D(x1, y1));
    ...
    [DebuggerStepThrough]
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static CubicBezier2D LineSegmentToCubicBezier(double x0, double y0, double x1, double y1) => new(new Point2D(x0, y0), Lerp(x0, y0, x1, y1, Floats<double>.OneThird), Lerp(x0, y0, x1, y1, Floats<double>.TwoThirds), new Point2D(x1, y1));
    ...
    public static Point2D[] QuadraticBezierToCubicBezierArray(Point2D a, Point2D b, Point2D c)
        ...
        [
            a,
            new(a.X + (Floats<double>.TwoThirds * (b.X - a.X)), a.Y + (Floats<double>.TwoThirds * (b.Y - a.Y))),
            new(c.X + (Floats<double>.TwoThirds * (b.X - c.X)), c.Y + (Floats<double>.TwoThirds * (b.Y - c.Y))),
            c
        ];
    ...
    public static CubicBezier2D QuadraticBezierToCubicBezier(Point2D a, Point2D b, Point2D c)
        => new(
            a.X, a.Y,
            a.X + (Floats<double>.TwoThirds * (b.X - a.X)), a.Y + (Floats<double>.TwoThirds * (b.Y - a.Y)),
            c.X + (Floats<double>.TwoThirds * (b.X - c.X)), c.Y + (Floats<double>.TwoThirds * (b.Y - c.Y)),
            c.X, c.Y
        );
    ...
    public static List<(double X, double Y)> QuadraticBezierToCubicBezier(double aX, double aY, double bX, double bY, double cX, double cY)
        ...
        [
            (aX, aY),
            (aX + (Floats<double>.TwoThirds * (bX - aX)), aY + (Floats<double>.TwoThirds * (bY - aY))),
            (cX + (Floats<double>.TwoThirds * (bX - cX)), cY + (Floats<double>.TwoThirds * (bY - cY))),
            (cX, cY)
        ];
    ...
    public static (double a, double b, double c, double d) CubicHermiteToCubicBezier(double p0, double t0, double p1, double t1)
    {
        ...
        var (m1x1, m1x2, m1x3, m1x4, m2x1, m2x2, m2x3, m2x4, m3x1, m3x2, m3x3, m3x4, m4x1, m4x2, m4x3, m4x4) = Floats<double>.InverseCubicBezierBernsteinBasisMatrix;
    ...
    public static (double a, double b) FindParabolaHFromTwoPointsAndK(double x1, double y1, double x2, double y2, double k)
    {
        ...
        if (u == 0d && v == 0d)
        {
            ...
            return (x2 + ((x2 - x1) * Floats<double>.OneHalf), double.NaN);
        }
        else if (u == 0d)
        {
        ...
        else
        {
            ...
            if (discriminant == 0)
            {
                return (Floats<double>.OneHalf * -b_, Floats<double>.OneHalf * -b_);
            }
            else if (discriminant > 0d)
            {
                var e = Sqrt(discriminant);
                return (Floats<double>.OneHalf * (-b_ + e), Floats<double>.OneHalf * (-b_ - e));
            }
            else
            {
