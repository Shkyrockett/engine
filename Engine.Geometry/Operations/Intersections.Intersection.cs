...
public static partial class Intersections
{
        ...
        double epsilon = double.Epsilon)
    {
        ...
        else if (discriminant == 0d)
        {
            ...
            var t = Floats<double>.OneHalf * -b / a;
        ...
        else if (discriminant > 0d)
        {
            ...
            var t1 = Floats<double>.OneHalf * (-b + Sqrt(discriminant)) / a;
            var t2 = Floats<double>.OneHalf * (-b - Sqrt(discriminant)) / a;
        ...
        double epsilon = double.Epsilon)
    {
        ...
        else if (discriminant > 0d)
        {
            ...
            var t1 = Floats<double>.OneHalf * (-b + root) / a;
            var t2 = Floats<double>.OneHalf * (-b - root) / a;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection QuadraticBezierSegmentQuadraticBezierSegmentIntersection(
        Polynomial<double> xCurveA, Polynomial<double> yCurveA,
        Polynomial<double> xCurveB, Polynomial<double> yCurveB,
        double epsilon = double.Epsilon)
    {
        // Initialize the intersection.
        var result = new Intersection(IntersectionStates.NoIntersection);
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection CircleCircleIntersection<T>(T h1, T k1, T r1, T h2, T k2, T r2, T? epsilon = default)
        where T : struct, IFloatingPointIeee754<T>
    {
        epsilon ??= T.Epsilon;

        var distance = T.Sqrt(((h2 - h1) * (h2 - h1)) + ((k2 - k1) * (k2 - k1)));
        switch (distance)
        {
            case T o when o > r1 + r2:
                // No intersection. 
                return new Intersection(IntersectionStates.Outside, Array.Empty<Point2D>());
            case T i when i < T.Abs(r1 - r2):
            ...
            default:
                ...
                var a = ((r1 * r1) - (r2 * r2) + (distance * distance)) / (Integers<T>.Two * distance);
                var x = h1 + ((h2 - h1) * (a / distance));
                var y = k1 + ((k2 - k1) * (a / distance));
                var b = T.Sqrt((r1 * r1) - (a * a)) / distance;
                // See if we have 1 or 2 solutions.
                return T.Abs(distance - r1 + r2) < epsilon
                    ? new Intersection(IntersectionStates.Intersection, new Point2D[] { (double.CreateSaturating(x + (b * (k2 - k1))), double.CreateSaturating(y - (b * (h2 - h1)))), })
                    : new Intersection(IntersectionStates.Intersection, new Point2D[] { (double.CreateSaturating(x + (b * (k2 - k1))), double.CreateSaturating(y - (b * (h2 - h1)))),
                                                                                        (double.CreateSaturating(x - (b * (k2 - k1))), double.CreateSaturating(y + (b * (h2 - h1)))), });
        }
    }
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection QuadraticBezierSegmentObliqueEllipseIntersection(Polynomial<double> xCurve, Polynomial<double> yCurve, double h, double k, double a, double b, double angle, double epsilon = double.Epsilon) => QuadraticBezierSegmentObliqueEllipseIntersection(xCurve, yCurve, h, k, a, b, Cos(angle), Sin(angle), epsilon);
    ...
    /// </acknowledgment>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection OrthogonalEllipseOrthogonalEllipseIntersection<T>(
        T h1, T k1, T a1, T b1,
        T h2, T k2, T a2, T b2,
        T? epsilon = default)
        where T : struct, IFloatingPointIeee754<T>
    {
        ...
        var yRoots = new Polynomial<T>(ConicSectionBezout(a, b)).Trim().Roots().ToArray();
        Array.Sort(yRoots);

        // Double epsilon is too small for here.
        epsilon = T.CreateSaturating(1e-6); //1e-3;
        var norm0 = ((a.a * a.a) + (Integers<T>.Two * a.b * a.b) + (a.c * a.c)) * epsilon;
        var norm1 = ((b.a * b.a) + (Integers<T>.Two * b.b * b.b) + (b.c * b.c)) * epsilon;

        var result = new Intersection(IntersectionStates.NoIntersection);
        for (var y = 0; y < yRoots.Length; y++)
        {
            var xRoots = new Polynomial<T>(
                 a.a,
                 a.d + (yRoots[y] * a.b),
            ...
            for (var x = 0; x < xRoots.Length; x++)
            {
                ...
                if (T.Abs(test) < norm0)
                {
                    ...
                    if (T.Abs(test) < norm1)
                    {
                        result.AppendPoint(new Point2D(double.CreateSaturating(xRoots[x]), double.CreateSaturating(yRoots[y])));
                    }
                }
            }
        }
    ...
    /// </acknowledgment>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection ObliqueEllipseObliqueEllipseIntersection<T>(T h1, T k1, T a1, T b1, T angle1, T h2, T k2, T a2, T b2, T angle2, T? epsilon = default) where T : struct, IFloatingPointIeee754<T> => ObliqueEllipseObliqueEllipseIntersection(h1, k1, a1, b1, T.Cos(angle1), T.Sin(angle1), h2, k2, a2, b2, T.Cos(angle2), T.Sin(angle2), epsilon);
    ...
    /// </acknowledgment>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection ObliqueEllipseObliqueEllipseIntersection<T>(
        T h1, T k1, T a1, T b1, T cos1, T sin1,
        T h2, T k2, T a2, T b2, T cos2, T sin2,
        T? epsilon = default)
        where T : struct, IFloatingPointIeee754<T>
    {
        epsilon ??= T.Epsilon;
        ...
        if (cos1 == Floats<T>.Cos0 && cos2 == Floats<T>.Cos0 && sin1 == Floats<T>.Sin0 && sin2 == Floats<T>.Sin0)
        {
        ...
        if (T.Abs(cos1 - cos2) < epsilon && T.Abs(sin1 + sin2) < epsilon && k1 == k2 && a1 == a2 && b1 == b2)
        {
            // Fix for Case 0.
            cos1 = cos2;
            sin1 = -sin2;
        ...
        var incidenceEpsilon = T.CreateSaturating(5e-16d);
        ...
        if (incidence == Incidence.Perpendicular && a1 == b2 && a2 == b1)
        {
            ...
            {
                // Rotate point.
                intersection.Points[i] = RotatePoint2D(intersection.Points[i].X, intersection.Points[i].Y, double.CreateSaturating(cos1), double.CreateSaturating(sin1), double.CreateSaturating(h1), double.CreateSaturating(k1));
            }
        ...
        var yRoots = new Polynomial<T>(ConicSectionBezout(e1, e2)).Trim().Roots().ToArray();
        Array.Sort(yRoots);

        // Double epsilon is too small for here.
        var normEpsilon = T.CreateSaturating(1e-3); //1e-3;
        var norm0 = ((e1.a * e1.a) + (Integers<T>.Two * e1.b * e1.b) + (e1.c * e1.c)) * normEpsilon;
        var norm1 = ((e2.a * e2.a) + (Integers<T>.Two * e2.b * e2.b) + (e2.c * e2.c)) * normEpsilon;
        ...
        foreach (var s in yRoots)
        {
            ...
            foreach (var t in xRoots)
            {
                ...
                if (T.Abs(test) < norm0)
                {
                    ...
                    if (T.Abs(test) < norm1)
                    {
                        result.AppendPoint(new Point2D(double.CreateSaturating(t), double.CreateSaturating(s)));
                    }
                }
            }
        }
    ...
    /// <summary>
    /// Finds the ellipse intersections.
    /// </summary>
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection FindEllipseIntersectionsParametric<T, R>(
        T a1, T b1, T h1, T k1, R theta1,
        T a2, T b2, T h2, T k2, R theta2,
        R? tolerance = default, R? stepSize = default)
        where T : struct, INumber<T>
        where R : struct, IFloatingPointIeee754<R>
    {
        stepSize ??= R.CreateSaturating(0.01);
        tolerance ??= R.CreateSaturating(1e-6);

        var result = new Intersection(IntersectionStates.NoIntersection);

        for (R t = R.Zero; t < R.CreateSaturating(2) * R.Pi; t += stepSize.Value)
        {
            var (x1, y1) = Interpolators.ParametricEllipseEquation(t, a1, b1, h1, k1, theta1);

            // Check if the point lies on the second ellipse within the tolerance
            if (R.Abs(Interpolators.ImplicitEllipseEquation(x1, y1, a2, b2, h2, k2, theta2)) < tolerance)
            {
                result.AppendPoint((double.CreateSaturating(x1), double.CreateSaturating(y1)));
            }
        }
    ...
    /// <summary>
    /// Finds the intersections newton raphson.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <typeparam name="R"></typeparam>
    /// <param name="a1">The a1.</param>
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection FindEllipseIntersectionsNewtonRaphson<T, R>(
        T a1, T b1, T h1, T k1, R theta1,
        T a2, T b2, T h2, T k2, R theta2,
        R? tolerance = default, int maxIterations = 100)
        where T : struct, INumber<T>
        where R : struct, IFloatingPointIeee754<R>
    {
        tolerance ??= R.CreateSaturating(1e-6);
        ...
        {
            var f1 = Interpolators.ImplicitEllipseEquation(x0, y0, a1, b1, h1, k1, theta1);
            var f2 = Interpolators.ImplicitEllipseEquation(x0, y0, a2, b2, h2, k2, theta2);

            if (R.Abs(f1) < tolerance && R.Abs(f2) < tolerance)
            {
                result.AppendPoint((double.CreateSaturating(x0), double.CreateSaturating(y0)));
                break;
            }

            var (f1x, f1y) = Interpolators.ImplicitEllipseEquationGradient(x0, y0, a1, b1, h1, k1, theta1);
            var (f2x, f2y) = Interpolators.ImplicitEllipseEquationGradient(x0, y0, a2, b2, h2, k2, theta2);

            var det = f1x * f2y - f1y * f2x;

            if (R.Abs(det) < R.CreateSaturating(1e-10))
            {
                break; // Jacobian is nearly singular
            }

            var dx = (f2y * f1 - f1y * f2) / det;
            var dy = (f1x * f2 - f2x * f1) / det;

            x0 -= dx;
            y0 -= dy;

            if (R.Sqrt(dx * dx + dy * dy) < tolerance)
            {
                result.AppendPoint((double.CreateSaturating(x0), double.CreateSaturating(y0)));
                break;
            }
    ...
    /// <summary>
    /// Finds the intersections polynomial.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <typeparam name="R"></typeparam>
    /// <param name="ea1">The ea1.</param>
    /// <param name="eb1">The eb1.</param>
    /// <param name="eh1">The eh1.</param>
    /// <param name="ek1">The ek1.</param>
    /// <param name="theta1">The theta1.</param>
    /// <param name="ea2">The ea2.</param>
    /// <param name="eb2">The eb2.</param>
    /// <param name="eh2">The eh2.</param>
    /// <param name="ek2">The ek2.</param>
    /// <param name="theta2">The theta2.</param>
    /// <param name="tolerance">The tolerance.</param>
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection FindEllipseIntersectionsPolynomial<T, R>(
        T ea1, T eb1, T eh1, T ek1, R theta1,
        T ea2, T eb2, T eh2, T ek2, R theta2,
        R? tolerance = default)
        where T : struct, INumber<T>
        where R : struct, IFloatingPointIeee754<R>
    {
        ...
        // Form the system of equations and solve
        // Here we assume simple ellipses and form the quadratic equation manually
        // This part needs to be adapted for the general case

        // Example for two simple ellipses centered at the origin without rotation
        var a1 = R.One / R.CreateSaturating(ea1 * ea1);
        var b1 = R.One / R.CreateSaturating(eb1 * eb1);
        var a2 = R.One / R.CreateSaturating(ea2 * ea2);
        var b2 = R.One / R.CreateSaturating(eb2 * eb2);

        // Form the resultant polynomial by eliminating y
        // Here we form the polynomial for x manually for simplicity
        // For a general case, use algebraic methods to eliminate variables

        // Solve for x (example for simple ellipses)
        var roots = QuadraticRoots(a1 - a2, R.Zero, b1 - b2);

        foreach (var x in roots)
        {
            var y1 = R.Sqrt((R.One - a1 * x * x) / b1);
            var y2 = -y1;

            // Verify solutions
            if (R.Abs(Interpolators.ImplicitEllipseEquation(x, y1, ea2, eb2, eh2, ek2, theta2)) < tolerance)
            {
                result.AppendPoint((double.CreateSaturating(x), double.CreateSaturating(y1)));
            }

            if (R.Abs(Interpolators.ImplicitEllipseEquation(x, y2, ea2, eb2, eh2, ek2, theta2)) < tolerance)
            {
                result.AppendPoint((double.CreateSaturating(x), double.CreateSaturating(y2)));
            }
        }
    ...
    /// <summary>
    /// Finds the ellipse intersections polynomial origin.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <typeparam name="R"></typeparam>
    /// <param name="ea1">The ea1.</param>
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static Intersection FindEllipseIntersectionsPolynomialOrigin<T, R>(
        T ea1, T eb1, T eh1, T ek1, R theta1,
        T ea2, T eb2, T eh2, T ek2, R theta2,
        R? tolerance = default)
        where T : struct, INumber<T>
        where R : struct, IFloatingPointIeee754<R>
    {
        ...
        // Transform ellipses to a common frame of reference
        (R h1, R k1) = Operations.TransformToEllipseOrigin(eh1, ek1, eh1, ek1, theta1);
        (R h2, R k2) = Operations.TransformToEllipseOrigin(eh2, ek2, eh1, ek1, theta1);
        var transformedE1 = (A: ea1, B: eb1, H: R.Zero, K: R.Zero, Theta: theta1 - theta1);
        var transformedE2 = (A: ea2, B: eb2, H: R.CreateSaturating(h2 - h1), K: R.CreateSaturating(k2 - k1), Theta: theta2 - theta1);

        // Form the resultant polynomial by eliminating y
        // Here we form the polynomial for x manually for simplicity
        // For a general case, use algebraic methods to eliminate variables
        var a1 = R.One / R.CreateSaturating(transformedE1.A * transformedE1.A);
        var b1 = R.One / R.CreateSaturating(transformedE1.B * transformedE1.B);
        var a2 = R.One / R.CreateSaturating(transformedE2.A * transformedE2.A);
        var b2 = R.One / R.CreateSaturating(transformedE2.B * transformedE2.B);
        ...
        {
            //if (root.Imaginary == 0) // Consider only real roots
            {
                var x = root/*.Real*/;

                // Calculate potential y values
                var y1 = R.Sqrt((R.One - x * x) / R.CreateSaturating(transformedE1.A * transformedE1.A * transformedE1.B * transformedE1.B));
                var y2 = -y1;

                // Verify solutions
                var (x1, y1T) = Operations.TransformFromEllipseOrigin(x, y1, transformedE1.H, transformedE1.K, transformedE1.Theta);
                var (x2, y2T) = Operations.TransformFromEllipseOrigin(x, y2, transformedE1.H, transformedE1.K, transformedE1.Theta);

                if (R.Abs(Interpolators.ImplicitEllipseEquation(x1, y1T, ea2, eb2, eh2, ek2, theta2)) < tolerance)
                {
                    result.AppendPoint((double.CreateSaturating(x1), double.CreateSaturating(y1T)));
                }

                if (R.Abs(Interpolators.ImplicitEllipseEquation(x2, y2T, ea2, eb2, eh2, ek2, theta2)) < tolerance)
                {
                    result.AppendPoint((double.CreateSaturating(x2), double.CreateSaturating(y2T)));
                }
            }
        }
