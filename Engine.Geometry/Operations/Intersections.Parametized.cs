...
namespace Engine
{
    ...
    public static partial class Intersections
    {
            ...
            double epsilon = Epsilon)
        {
            ...
            var ub = (ri * (ry - ly)) - (rj * (rx - lx)); // Ray2D
            ...
            if (Abs(determinant) < epsilon)
            {
                if (ua == 0d || ub == 0d)
                {
                    // Ray2D is coincident to the Line. There are an infinite number of intersections, but we only care about the start and end points of the line segment.
