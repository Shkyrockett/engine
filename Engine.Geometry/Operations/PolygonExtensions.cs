...
public static class PolygonExtensions
{
    ...
    /// </acknowledgment>
    public static Polyline2D ShortestPath(this Polygon2D polygons, Point2D start, Point2D end)
    {
        ...
        while (bestJ < pointList.Count - 1)
        {
            ...
            if (Abs(bestDist - maxLength) < double.Epsilon)
            {
    ...
    /// http://csharphelper.com/blog/2014/07/perform-geometric-operations-on-polygons-in-c/
    /// </acknowledgment>
    private static void OrientPolygonClockwise(this PolygonContour2D polygon)
    {
    ...
    /// http://csharphelper.com/blog/2014/07/perform-geometric-operations-on-polygons-in-c/
    /// </acknowledgment>
    public static bool PolygonIsConvex(this PolygonContour2D polygon)
    {
    ...
    /// http://csharphelper.com/blog/2014/07/perform-geometric-operations-on-polygons-in-c/
    /// </acknowledgment>
    private static (int A, int B, int C) FindEar(this PolygonContour2D polygon)
    {
        var num_points = polygon.Points.Count;
        int A;
        var B = 0;
        ...
        for (A = 0; A < num_points; A++)
        {
            ...
            if (FormsEar([.. polygon.Points], A, B, C))
            {
                return (A, B, C);
            }
        }
    ...
    /// http://csharphelper.com/blog/2014/07/perform-geometric-operations-on-polygons-in-c/
    /// </acknowledgment>
    private static void RemoveEar(this PolygonContour2D polygon, List<Triangle2D> triangles)
    {
        // Find an ear.
        (var a, var b, var c) = FindEar(polygon);

        // Create a new triangle for the ear.
        triangles.Add(new Triangle2D(polygon.Points[a], polygon.Points[b], polygon.Points[c]));
    ...
    /// http://csharphelper.com/blog/2014/07/perform-geometric-operations-on-polygons-in-c/
    /// </acknowledgment>
    private static void RemovePoint2DFromArray(this PolygonContour2D polygon, int target) => polygon.Points.RemoveAt(target);
    ...
    public static List<Triangle2D> Triangulate(this PolygonContour2D polygon)
    {
        ...
        var pgon = new PolygonContour2D(new List<Point2D>(pts));
    ...
    /// <param name="polygon">The polygon.</param>
    /// <returns></returns>
    public static IEnumerable<LineSegment2D> IterateEdgesClockwise(params List<Point2D> polygon)
    {
    ...
    public static bool IsInside(LineSegment2D edge, Point2D test)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public static bool IsClockwise(params List<Point2D> polygon)
    {
