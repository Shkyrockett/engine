    ...
    : Shape2D, IShapeSegment
{
    ...
    /// </summary>
    public static readonly LineSegment2D Empty = new(0d, 0d, 0d, 0d);
    ...
    public Polynomial<double> CurveX
    {
        get
        {
            var curveX = (Polynomial<double>)CachingProperty(() => Polynomials.Bezier(Points.Select(p => p.X).ToArray()));
    ...
    public Polynomial<double> CurveY
    {
        get
        {
            var curveY = (Polynomial<double>)CachingProperty(() => Polynomials.Bezier(Points.Select(p => p.Y).ToArray()));
    ...
    public static implicit operator LineSegment2D((double I, double J, double K, double L) tuple) => new(tuple);
    #endregion Operators
    ...
    public Point2D[] ToArray() => [A, B];

    /// <summary>
    /// The to line.
    /// </summary>
    /// <returns>The <see cref="Line2D"/>.</returns>
    public Line2D ToLine() => new(aX, aY, aX - bX, aY - bY);
