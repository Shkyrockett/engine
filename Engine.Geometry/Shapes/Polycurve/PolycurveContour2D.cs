...
namespace Engine
{
        ...
        : Shape2D, IEnumerable<CurveSegment2D>
    {
        ...
        public PolycurveContour2D(List<CurveSegment2D> items)
        {
            this.items = items;
        }
        ...
        public PolycurveContour2D(CubicBezier2D[] curves)
        {
            if (curves is null)
            {
            ...
            items = new List<CurveSegment2D> { new PointSegment2D(curves[0].A) };
            ...
            foreach (var token in Regex.Split(pathDefinition, separators).Where(t => !string.IsNullOrWhiteSpace(t)))
            {
                ...
                switch (cmd)
                {
                    ...
                    case 'l': // Relative Svg lineto
                        ...
                        goto case 'L';
                    case 'L': // Svg lineto
                        item = new LineCurveSegment2D(item, relitive, args);
                        figure.Add(item);
                        item.Relitive = relitive;
                        relitive = false;
                        break;
