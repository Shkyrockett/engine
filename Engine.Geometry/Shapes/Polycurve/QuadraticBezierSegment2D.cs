     ...
     : CurveSegment2D
{
    ...
    public QuadraticBezierSegment2D(CurveSegment2D previous, bool relative, double[] args)
        : this(previous, relative, args?.Length == 4 ? new Point2D[] { new(args[0], args[1]), new(args[2], args[3]) }
            : args?.Length == 2 ? [new(args[0], args[1])] : null)
    { }
    ...
    public override List<Point2D> Grips => [Head, Handle, Tail];
    ...
    public Polynomial<double> CurveX
    {
        get
        {
            var curveX = (Polynomial<double>)CachingProperty(() => (Polynomial<double>)QuadraticBezierBernsteinBasis(Head.X, Handle.X, Tail.X));
    ...
    public Polynomial<double> CurveY
    {
        get
        {
            var curveY = (Polynomial<double>)CachingProperty(() => (Polynomial<double>)QuadraticBezierBernsteinBasis(Head.Y, Handle.Y, Tail.Y));
    ...
    public QuadraticBezier2D ToQuadtraticBezier() => new(Head, Handle, Tail);
    #endregion Methods
}
