...
namespace Engine.Winforms
{
        ...
        : PictureBox
    {
        ...
        {
            int index = Palette.PointToPaletteEntry(e.Location, ClientRectangle);
            if (index >= 0)
            {
                switch (e.Button)
                {
        ...
        private void PalleteControl_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            int index = Palette.PointToPaletteEntry(e.Location, new Rectangle(ClientRectangle.Location, new Size(ClientRectangle.Width - 1, ClientRectangle.Height - 1)));
