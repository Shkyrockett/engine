...
namespace Engine
{
    ...
    [DebuggerDisplay("{nameof(Matrix2x2D)}({nameof(M0x0)}: {M0x0 ?? double.NaN}, {nameof(M0x1)}: {M0x1 ?? double.NaN}, {nameof(M0x2)}: {M0x2 ?? double.NaN}, {nameof(M0x3)}: {M0x3 ?? double.NaN}, {nameof(M1x0)}: {M1x0 ?? double.NaN}, {nameof(M1x1)}: {M1x1 ?? double.NaN}, {nameof(M1x2)}: {M1x2 ?? double.NaN}), {nameof(M1x3)}: {M1x3 ?? double.NaN}), {nameof(M2x0)}: {M2x0 ?? double.NaN}, {nameof(M2x1)}: {M2x1 ?? double.NaN}, {nameof(M2x2)}: {M2x2 ?? double.NaN}, {nameof(M2x3)}: {M2x3 ?? double.NaN}, {nameof(M3x0)}: {M3x0 ?? double.NaN}, {nameof(M3x1)}: {M3x1 ?? double.NaN}, {nameof(M3x2)}: {M3x2 ?? double.NaN}, {nameof(M3x3)}: {M3x3 ?? double.NaN})")]
        ...
        : IMatrix<Matrix4x4D, Vector4D>
    {
        ...
        [DebuggerStepThrough]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Equals(Matrix4x4D matrix2)
            => M0x0 == matrix2.M0x0
            && M0x1 == matrix2.M0x1
            && M0x2 == matrix2.M0x2
            && M0x3 == matrix2.M0x3
