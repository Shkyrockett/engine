using System;
using System.Diagnostics;
using System.Drawing;
					
public class Program
{
	public static void Main()
	{
		var watch = new Stopwatch();
		var trials = 10000;
		
		int TotalRunningTime;
		double AverageRunningTime;
		
		var points = new PointF[]{
			new PointF(1, 1),
			new PointF(2, 1),
			new PointF(3, 1),
			new PointF(4, 1),
			new PointF(5, 1),
			new PointF(6, 1),
			new PointF(7, 1),
			new PointF(8, 1),
			new PointF(9, 1),
			new PointF(10, 1),
			new PointF(11, 1),
			new PointF(12, 1),
			new PointF(13, 1),
			new PointF(14, 1),
			new PointF(15, 1),
			new PointF(16, 1),
			new PointF(17, 1),
			new PointF(18, 1),
			new PointF(19, 1),
			new PointF(20, 1),
			new PointF(21, 1),
			new PointF(22, 1),
			new PointF(23, 1),
			new PointF(24, 1),
			new PointF(25, 1),
		};
		
		float result = 0;
		
		watch.Reset();
		watch.Start();
		
		for (int z = 0; z < trials ; z++)
		for (int i = 1; i < points.Length; i++)
		{
    		result = ProcessSegment(points[i - 1], points[i]);
		}
		
		watch.Stop();
		
		TotalRunningTime = (int)watch.ElapsedMilliseconds;
        AverageRunningTime = (double)TotalRunningTime / (trials * points.Length);
		Console.WriteLine($"[ProcessSegment(points[i - 1], points[i])]\n\rTotal running time:\t{TotalRunningTime}\n\rAverage running time:\t{AverageRunningTime}\n\r" );
		
		watch.Reset();
		watch.Start();
		
		for (int z = 0; z < trials ; z++)
		for (int i = 0; i < points.Length; i++)
		{
    		result = ProcessSegment(points[i], points[(i + 1) % points.Length]);
		}
		
		watch.Stop();
		
		TotalRunningTime = (int)watch.ElapsedMilliseconds;
        AverageRunningTime = (double)TotalRunningTime / (trials * points.Length);
		Console.WriteLine($"[ProcessSegment(points[i], points[(i + 1) % points.Length])]\n\rTotal running time:\t{TotalRunningTime}ms.\n\rAverage running time:\t{AverageRunningTime}\n\r" );
		
		watch.Reset();
		watch.Start();
		
		for (int z = 0; z < trials ; z++)
		for (int i = 0; i < points.Length; i++)
		{
    		result = (i < points.Length - 1)? ProcessSegment(points[i], points[i + 1]): ProcessSegment(points[i], points[(i + 1) % points.Length]);
		}
		
		watch.Stop();
		
		TotalRunningTime = (int)watch.ElapsedMilliseconds;
        AverageRunningTime = (double)TotalRunningTime / (trials * points.Length);
		Console.WriteLine($"[?ProcessSegment(points[i], points[(i + 1) % points.Length]]\n\rTotal running time:\t{TotalRunningTime}ms.\n\rAverage running time:\t{AverageRunningTime}\n\r" );
		
		
		watch.Reset();
		watch.Start();
		
		for (int z = 0; z < trials ; z++)
		for (int i = 0; i < points.Length; i++)
		{
    		if (i < points.Length - 1)
				result = ProcessSegment(points[i], points[i + 1]);
			else
				result = ProcessSegment(points[i], points[(i + 1) % points.Length]);
		}
		
		watch.Stop();
		
		TotalRunningTime = (int)watch.ElapsedMilliseconds;
        AverageRunningTime = (double)TotalRunningTime / (trials * points.Length);
		
		Console.WriteLine($"[if ProcessSegment(points[i], points[(i + 1) % points.Length]]\n\rTotal running time:\t{TotalRunningTime}ms.\n\rAverage running time:\t{AverageRunningTime}\n\r" );
				
		watch.Reset();
		watch.Start();
		
		for (int z = 0; z < trials ; z++)
		for (int i = 0; i < points.Length-1; i++)
		{
			result = ProcessSegment(points[i], points[i + 1]);	
		}
		result = ProcessSegment(points[points.Length - 1], points[0]);
		
		watch.Stop();
		
		TotalRunningTime = (int)watch.ElapsedMilliseconds;
        AverageRunningTime = (double)TotalRunningTime / (trials * points.Length);
		
		Console.WriteLine($"[ProcessSegment(points[points.Length-1], points[0])]\n\rTotal running time:\t{TotalRunningTime}ms.\n\rAverage running time:\t{AverageRunningTime}\n\r" );
		
			
		watch.Reset();
		watch.Start();
		
		for (int z = 0; z < trials ; z++)
		for (int i = 01; i < points.Length; i++)
		{
			result = ProcessSegment(points[i - 1], points[i]);	
		}
		result = ProcessSegment(points[points.Length - 1], points[0]);
		
		watch.Stop();
		
		TotalRunningTime = (int)watch.ElapsedMilliseconds;
        AverageRunningTime = (double)TotalRunningTime / (trials * points.Length);
		
		Console.WriteLine($"[ProcessSegment(points[points.Length-1], points[0])]\n\rTotal running time:\t{TotalRunningTime}ms.\n\rAverage running time:\t{AverageRunningTime}\n\r" );
	}
	
	public static float ProcessSegment(PointF a, PointF b)
	{
		return a.X * b.X - a.Y * b.Y;
	}	
}